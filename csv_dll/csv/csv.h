...
#ifdef CSV_EXPORTS
#define CSV_EXPORTS _declspec(dllexport)
#else
#define CSV_EXPORTS _declspec(dllimport)
#endif // !LIBCSVDLL
...
/* Function Prototypes */
CSV_EXPORTS int csv_init(struct csv_parser *p, unsigned char options);
CSV_EXPORTS int csv_fini(struct csv_parser *p, void (*cb1)(void *, size_t, void *), void (*cb2)(int, void *), void *data);
CSV_EXPORTS void csv_free(struct csv_parser *p);
CSV_EXPORTS int csv_error(const struct csv_parser *p);
CSV_EXPORTS const char * csv_strerror(int error);
CSV_EXPORTS size_t csv_parse(struct csv_parser *p, const void *s, size_t len, void (*cb1)(void *, size_t, void *), void (*cb2)(int, void *), void *data);
size_t csv_write(void *dest, size_t dest_size, const void *src, size_t src_size);
CSV_EXPORTS int csv_fwrite(FILE *fp, const void *src, size_t src_size);
